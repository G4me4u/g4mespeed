...
public class GSScrollBar extends DrawableHelper implements Drawable {
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		drawScrollButton(matrixStack, mouseX, mouseY, x + width - getButtonSizeX(), y + height - getButtonSizeY(), false);
		
		drawKnobArea(matrixStack);
		drawKnob(matrixStack, mouseX, mouseY);
	}
	...
	private void drawScrollButton(MatrixStack matrixStack, int mouseX, int mouseY, int bx, int by, boolean top) {
		...
		drawTexture(matrixStack, bx, by, sx * 10, sy * 10, bsx, bsy, 30, 40);
	}
	...
	private void drawKnobArea(MatrixStack matrixStack) {
		...
		fill(matrixStack, x0, y0, x1, y1, 0xFF000000);
		...
		fill(matrixStack, x0, y0, x1, y1, getKnobAreaColor());
	}
	
	private void drawKnob(MatrixStack matrixStack, int mouseX, int mouseY) {
		...
		fill(matrixStack, kx, ky, kx + kw, ky + kh, getKnobColor(isMouseOverKnob(mouseX, mouseY)));
	}
