...
public class GSBasicRenderer2D implements GSIRenderer2D {
	...
	@Override
	public void beginRendering(int mouseX, int mouseY, float partialTicks) {
		this.mouseX = mouseX;
		this.mouseY = mouseY;
	...
	public void fillRectGradient(int x, int y, int width, int height, int tlColor, int brColor) {
		...
		bufferBuilder.vertex( x, y1, 0.0f).color(r1, g1, b1, a1).next();
	...
	public void drawTexture(GSITextureRegion texture, int x, int y) {
		...
		bufferBuilder.vertex(x , y1, 0.0f).texture(texture.getU0(), texture.getV1()).next();
		bufferBuilder.vertex(x1, y1, 0.0f).texture(texture.getU1(), texture.getV1()).next();
	...
	public float getStringWidth(String str) {
		return client.textRenderer.getStringWidth(str);
	}
	...
	public void drawString(String str, int x, int y, int color, boolean shadowed) {
		...
		if (shadowed) {
			client.textRenderer.drawWithShadow(str, x, y, color);
		} else {
