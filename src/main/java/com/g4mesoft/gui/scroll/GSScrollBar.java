...
public class GSScrollBar extends GSPanel {
	...
	public void renderTranslated(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		drawScrollButtons(matrixStack, mouseX, mouseY);
		
		drawKnobArea(matrixStack);
		drawKnob(matrixStack, mouseX, mouseY, isMouseOverKnob(mouseX, mouseY));
	}
	
	protected void drawScrollButtons(MatrixStack matrixStack, int mouseX, int mouseY) {
		client.getTextureManager().bindTexture(getScrollButtonTexture());
		
		GlStateManager.disableBlend();
		GlStateManager.color4f(1.0f, 1.0f, 1.0f, 1.0f);
		
		drawScrollButton(matrixStack, mouseX, mouseY, true);
		drawScrollButton(matrixStack, mouseX, mouseY, false);
	}

	protected void drawScrollButton(MatrixStack matrixStack, int mouseX, int mouseY, boolean top) {
		int bp = top ? 0 : (isVertical() ? height : width) - getButtonHeight();
		boolean hovered = isMouseOverScrollButton(mouseX, mouseY, top);

		if (isVertical()) {
			drawScrollButton(matrixStack, 0, bp, getButtonWidth(), getButtonHeight(), top, hovered);
		} else {
			drawScrollButton(matrixStack, bp, 0, getButtonHeight(), getButtonWidth(), top, hovered);
		}
	}
	
	protected void drawScrollButton(MatrixStack matrixStack, int bx, int by, int bw, int bh, boolean top, boolean hovered) {
		int sx = getScrollButtonSpriteX(top, hovered);
		int sy = getScrollButtonSpriteY(top, hovered);
		drawTexture(matrixStack, bx, by, sx, sy, bw, bh, getScrollButtonSpriteWidth(), getScrollButtonSpriteHeight());
	}
	...
	protected void drawKnobArea(MatrixStack matrixStack) {
		...
		if (isVertical()) {
			fill(matrixStack, 0, buttonHeight, width, height - buttonHeight, 0xFF000000);
			fill(matrixStack, 1, buttonHeight, width - 1, height - buttonHeight, getKnobAreaColor());
		} else {
			fill(matrixStack, buttonHeight, 0, width - buttonHeight, height, 0xFF000000);
			fill(matrixStack, buttonHeight, 1, width - buttonHeight, height - 1, getKnobAreaColor());
		}
	}

	protected void drawKnob(MatrixStack matrixStack, int mouseX, int mouseY, boolean hovered) {
		int k0 = getKnobPos();
		...
		if (isVertical()) {
			fill(matrixStack, 1, k0, width - 1, k1, color);
		} else {
			fill(matrixStack, k0, 1, k1, height - 1, color);
		}
	}
