...
public class GSBooleanSettingPanel extends GSSettingPanel<GSBooleanSetting> {
	...
	private static final Text ENABLED_TEXT = GSTextUtil.translatable("setting.boolean.enabled").setStyle(new Style().setColor(Formatting.GREEN));
	private static final Text DISABLED_TEXT = GSTextUtil.translatable("setting.boolean.disabled").setStyle(new Style().setColor(Formatting.RED));
