...
public abstract class GSFallingBlockEntityMixin extends Entity {
	...
	@Redirect(
		...
		at = @At(
			value = "INVOKE",
			target =
				"Lnet/minecraft/server/level/ChunkMap;broadcast(" +
					"Lnet/minecraft/world/entity/Entity;" +
					"Lnet/minecraft/network/protocol/Packet;" +
				")V"
		)
	)
	private void redirectSendToOtherNearbyPlayers(ChunkMap chunkStorage, Entity entity, Packet<?> packet) {
		Level level = level();
		if (level.isClientSide || GSServerController.getInstance().getTpsModule().sPrettySand.get() == GSTpsModule.PRETTY_SAND_DISABLED)
			chunkStorage.broadcast(entity, packet);
	}
